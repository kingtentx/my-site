...
第三方库包括 `layui-v2.6.8`、`ssi-uploader`、`wangEditor-4.7.13`，以及 jQuery、Sortable 等独立插件。`wangEditor-4.7.13/` 仅保留浏览器可直接加载的编译文件及许可证，不存放源码包。后台框架内置的 `layuiadmin/layui/` 随框架整体保留，避免破坏其相对路径。新增第三方库应优先放入 `plugin/`，不要与站点自有代码混放。
