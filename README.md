...
已有子页面的页面作为导航目录，不再装修或展示自身内容。目录仍需发布；前台点击目录时会进入按排序排列的首个已发布子页面（优先选择显示在导航中的子页面）。原有装修草稿会保留；若没有已发布子页面，目录不会出现在前台导航中。
...
- 右侧属性面板统一使用已部署的 wangEditor 4.7.13 编译文件，提供标题、加粗／斜体、字号、文字颜色、对齐、列表、引用、链接、图片、表格及编辑器撤销／重做。编辑内容实时同步到装修画布，保存后用于预览和正式页面。
- 字号、颜色和对齐转换为预定义样式类；客户端和服务端仅保留白名单标签、样式类、安全链接与图片地址，不接受任意内联 CSS。上传图片沿用后台的 `/Upload/UploadImage` 接口。
- 浏览器回归页：`scripts/smoke-richtext-toolbar.html`（在 Edge/Chrome 打开，页面标题显示 PASS/FAIL）。
