...
namespace CIMC.Helper.SM4
{
    public class SM4CryptoServiceProvider
    {
        ...
        private void sm4_setkey(int[] SK, byte[] key)
        {
            ...
            int[] k = new int[36];
            int i = 0;
            MK[0] = GET_Uint_BE(key, 0);
            MK[1] = GET_Uint_BE(key, 4);
            MK[2] = GET_Uint_BE(key, 8);
            MK[3] = GET_Uint_BE(key, 12);
            k[0] = MK[0] ^ (int)FK[0];
            k[1] = MK[1] ^ (int)FK[1];
            k[2] = MK[2] ^ (int)FK[2];
            k[3] = MK[3] ^ (int)FK[3];
        ...
        private void sm4_one_round(int[] sk, byte[] input, byte[] output)
        {
            ...
            ulbuf[0] = GET_Uint_BE(input, 0);
            ulbuf[1] = GET_Uint_BE(input, 4);
            ulbuf[2] = GET_Uint_BE(input, 8);
            ulbuf[3] = GET_Uint_BE(input, 12);
            while (i < 32)
            {
