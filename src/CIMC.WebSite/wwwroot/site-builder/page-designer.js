(function (window, document, $) {
    ...
    function groupName(key) {
        return { layout:'布局组件', basic:'基础组件', data:'内容组件', global:'全局组件' }[key] || key;
    }

    function renderLibrary() {
        var groups = SB.Registry.groups();
        var order = ['layout','basic','data','global'];
    ...
    function selectedParentId() {
        ...
        if (node && SB.Tree.canHaveChildren(node) && !node.locked) return node.id;
        return null;
    }
    ...
    function addPreset(key) {
        ...
        store.change(function (doc) {
            if (!SB.Tree.insert(doc.nodes, node, parentId)) SB.Tree.insert(doc.nodes, node, null);
        });
        store.select(node.id);
    }
    ...
    function load() {
        $.get('/Page/GetComponentData', { pageId: config.pageId }).done(function (res) {
            if (!res || Number(res.code) !== 0) {
                message((res && res.message) || '读取页面结构失败', 2);
                return;
            }
            try {
                var doc = res.document;
                if (!doc) doc = { schemaVersion:1, name:config.pageName || '', nodes:[], settings:{} };
                store.load(doc);
            } catch (e) {
                store = new SB.Store(config.pageName || '');
                store.subscribe(render);
                render();
                message(e.message + ' 已创建空白新版页面。', 0);
            }
    ...
    function saveDraft(done) {
        $.ajax({
            type: 'post',
            url: '/Page/SaveDraft',
            data: { id: config.pageId, documentJson: store.serialize() }
        }).done(function (res) {
            if (!res || Number(res.code) !== 0) { message((res && res.message) || '保存失败', 2); return; }
            message('草稿已保存', 1);
            if (done) done();
    ...
    function bindEvents() {
        ...
        $('#canvas').on('click', '.sb-node', function (e) {
            if ($(e.target).closest('.sb-node-actions').length) return;
            e.stopPropagation();
            store.select($(this).attr('data-node-id'));
        });
        ...
        $('#canvas').on('click', '.sb-node-actions button', function (e) {
            ...
            var id = $(this).closest('.sb-node').attr('data-node-id');
            var node = SB.Tree.find(store.document.nodes, id);
            if (!node) return;
            var action = $(this).attr('data-action');
            if (node.locked && action !== 'toggle') { message('组件已锁定，请先在右侧取消锁定'); return; }
            if (action === 'delete') store.remove(id);
            if (action === 'duplicate') store.duplicate(id);
            if (action === 'toggle') store.update(id, 'node', 'visible', node.visible === false);
        });

        $('#propsPanel').on('change', '[data-area][data-key]', function () {
            var node = store.selected();
            if (!node) return;
            var area = $(this).attr('data-area'), key = $(this).attr('data-key');
            store.update(node.id, area, key, SB.Inspector.readValue(this));
        });
        ...
        $('#btnPublish').on('click', function () {
            saveDraft(function () {
                $.post('/Page/Publish', { id: config.pageId }).done(function (res) {
                    if (res && Number(res.code) === 0) message('页面已发布', 1);
                    else message((res && res.message) || '发布失败', 2);
                });
            });
        });
    }
    ...
    if (window.layui) {
        layui.use(['layer','form'], function () { layer = layui.layer; bindEvents(); renderLibrary(); store.subscribe(render); render(); load(); });
    } else {
        bindEvents(); renderLibrary(); store.subscribe(render); render(); load();
    }
