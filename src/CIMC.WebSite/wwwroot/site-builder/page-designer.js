(function (window, document, $) {
    ...
    function ensureDesignerRuntimeAssets() {
        ...
        addCss('sbPublicSiteCss', '/site/css/site.css?v=2026090507');
        addCss('sbRuntimeCss', '/site-builder/runtime.css?v=2026090507');
        ...
        if (!document.getElementById('sbDesignerWysiwygCss')) {
            ...
            style.textContent = [
                ...
                '#canvas .sb-node{position:relative!important;margin:0!important;border:0!important;border-radius:0!important;background:transparent!important;outline:1px dashed transparent;box-shadow:none;transition:outline-color .12s,box-shadow .12s}',
                '#canvas .sb-node:hover{outline:1px dashed #60a5fa}',
                '#canvas .sb-node.selected{outline:2px solid #1677ff!important;box-shadow:0 0 0 3px rgba(22,119,255,.08)!important}',
                '#canvas .sb-node.is-hidden{opacity:.42}',
                '#canvas .sb-children{padding:0!important}',
                '#canvas .sb-container-node{min-height:34px}',
                '#canvas .sb-leaf-node{min-width:0}',
                '#canvas .sb-leaf-content{min-width:0}',
                '#canvas .sb-grid>.sb-node{min-width:0}',
                '#canvas .sb-node-toolbar{position:absolute!important;left:4px!important;right:4px!important;top:4px!important;height:28px!important;z-index:60!important;display:flex!important;align-items:center!important;padding:0 6px!important;border:1px solid #bfdbfe!important;border-radius:4px!important;background:rgba(248,250,252,.96)!important;box-shadow:0 2px 7px rgba(15,23,42,.08)!important;font-size:11px!important;opacity:0;pointer-events:none;transition:opacity .12s}',
                '#canvas .sb-node:hover>.sb-node-toolbar,#canvas .sb-node.selected>.sb-node-toolbar{opacity:1;pointer-events:auto}',
                '#canvas .sb-drag{cursor:grab;font-weight:600;flex:1;user-select:none;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
                '#canvas .sb-drag:active{cursor:grabbing}',
                '#canvas .sb-node-type{font-size:9px;color:#94a3b8;margin-right:6px}',
                '#canvas .sb-node-actions{display:flex!important;gap:3px}',
                '#canvas .sb-node-actions button{height:21px;border:1px solid #dbe1e8;background:#fff;border-radius:3px;padding:0 5px;font-size:10px;cursor:pointer}',
                ...
                '#canvas a,#canvas button:not(.sb-node-actions button):not(.sb-grid-resize-handle),#canvas input,#canvas video{pointer-events:none}',
    ...
    function ensureCanvasViewport() {
        ...
        if (!$canvas.parent().hasClass('sb-canvas-scale-frame')) {
            ...
            $canvas.after('<div class="sb-wysiwyg-note">画布与前台使用同一套组件样式；Header/Footer 在“保存并预览”中按全站配置显示</div>');
        }
        if (!$('#sbCanvasWidth').length) {
            var suggested = Math.max(1200, Math.min(1920, Number(window.screen && window.screen.width ? window.screen.width : 1440)));
            var saved = Number(window.localStorage && localStorage.getItem('siteBuilderCanvasWidth')) || suggested;
            var options = [1200, 1440, 1920];
            var nearest = options.reduce(function (best, item) { return Math.abs(item - saved) < Math.abs(best - saved) ? item : best; }, 1440);
            var html = '<div class="sb-viewport-tools"><span>设计宽度</span><select id="sbCanvasWidth">';
        ...
        if (window.ResizeObserver && !canvasResizeObserver) {
            canvasResizeObserver = new ResizeObserver(function () { applyCanvasViewport(true); });
    ...
    function renderLibrary() {
        ...
        var html = '<div class="lib-tip">组件结构与发布页共用同一套运行时样式。建议先添加区段/容器/网格，再放入内容组件。</div>';
    ...
    function load() {
        $.get('/Page/GetComponentData', { pageId:config.pageId }).done(function (res) {
            ...
            try { store.load(res.document || {schemaVersion:1,name:config.pageName||'',nodes:[],settings:{}}); }
    ...
    function previewUrl() {
        return String(config.pagePath || '').indexOf('/__global/') === 0
            ? '/Page/Preview?id=' + config.pageId
            : '/Home/BuilderPreview?id=' + config.pageId;
    }
    ...
    function bindEvents() {
        ...
        $('#canvas').on('click.siteBuilder','.sb-node-actions button',function(e){
            e.preventDefault();e.stopPropagation();var id=$(this).closest('.sb-node').attr('data-node-id'),node=SB.Tree.find(store.document.nodes,id);if(!node)return;var action=$(this).attr('data-action');
            if(node.locked&&action!=='toggle'){message('组件已锁定，请先在右侧取消锁定');return;}if(action==='delete')store.remove(id);else if(action==='duplicate')store.duplicate(id);else if(action==='toggle')store.update(id,'node','visible',node.visible===false);
        });
        ...
        $(document).on('change.siteBuilderViewport','#sbCanvasWidth',function(){canvasWidth=Number($(this).val())||1440;try{localStorage.setItem('siteBuilderCanvasWidth',String(canvasWidth));}catch(e){}applyCanvasViewport();});
        ...
        $('#btnPreview').on('click.siteBuilder',function(){saveDraft(function(){window.open(previewUrl(),'_blank');});});
        $('#btnPublish').on('click.siteBuilder',function(){saveDraft(function(){$.post('/Page/Publish',{id:config.pageId}).done(function(res){if(ok(res))message('页面已发布',1);else message((res&&res.message)||'发布失败',2);});});});
    }
