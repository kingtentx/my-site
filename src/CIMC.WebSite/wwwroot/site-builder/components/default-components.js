(function (window) {
    ...
    reg({ type: 'productList', name: '产品列表', group: 'data', icon: 'layui-icon-component', desc: '绑定后台产品数据', styleScope: SCOPE_BOX, defaults: { categoryIds: [], pageSize: 8, columns: 4, showSummary: false }, inspector: [f('categoryIds', '选择分类', 'categories', { contentType: 'product', allText: '全部产品', hint: '点「选择分类」从产品分类中勾选；不勾选 = 全部产品，可多选' }), f('pageSize', '显示数量', 'number', { min: 1, max: 50 }), f('columns', '列数', 'number', { min: 1, max: 6 }), f('showSummary', '显示摘要', 'checkbox')] });
    reg({ type: 'jobList', name: '招聘列表', group: 'data', icon: 'layui-icon-friends', desc: '绑定后台招聘数据', styleScope: SCOPE_BOX, defaults: { categoryIds: [], pageSize: 10, showSalary: true, showLocation: true }, inspector: [f('categoryIds', '选择分类', 'categories', { contentType: 'job', allText: '全部招聘', hint: '点「选择分类」从招聘分类中勾选；不勾选 = 全部招聘，可多选' }), f('pageSize', '显示数量', 'number', { min: 1, max: 50 }), f('showSalary', '显示薪资', 'checkbox'), f('showLocation', '显示地点', 'checkbox'), f('layout', '列表样式', 'select', { options: [{ value: '', text: '标准列表' }, { value: 'compact', text: '简洁岗位列表' }] })] });
