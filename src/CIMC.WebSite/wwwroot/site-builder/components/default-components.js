(function (window) {
    ...
    reg({ type: 'articleList', name: '文章列表', group: 'data', icon: 'layui-icon-list', desc: '绑定后台新闻数据，支持多种展示样式与分页', styleScope: SCOPE_BOX, defaults: { categoryId: 0, pageSize: 6, columns: 3, showSummary: true, showDate: true, showImage: true, enablePagination: true, layout: 'card' }, inspector: [f('categoryId', '选择分类', 'category', { contentType: 'article', allText: '全部分类' }), f('layout', '展示样式', 'select', { options: [{ value: 'card', text: '网格卡片' }, { value: 'list', text: '纵向列表' }, { value: 'timeline', text: '时间轴' }, { value: 'editorial', text: '企业新闻（大图首条）' }] }), f('pageSize', '每页数量', 'number', { min: 1, max: 100 }), f('columns', '列数', 'number', { min: 1, max: 6, hint: '仅在网格卡片样式生效' }), f('showSummary', '显示摘要', 'checkbox'), f('showDate', '显示日期', 'checkbox'), f('showImage', '显示封面图', 'checkbox'), f('enablePagination', '开启分页', 'checkbox')] });
