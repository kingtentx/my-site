...
namespace CimcSite.Web.Controllers
{
    ...
    {
        private readonly IRepository<Attachments> _repository;
        private readonly IPermissionService _permission;
        private readonly IWebHostEnvironment _environment;

        public AttachmentsController(IRepository<Attachments> repository, IPermissionService permission, IWebHostEnvironment environment)
        {
        ...
        [PermissionFilter(MenuCode.Content_Attachments, PermissionType.View)]
        public IActionResult Index()
        {
            ...
            ViewData[PageCode.PAGE_Button_Edit] = _permission.CheckPermission(LoginUser, MenuCode.Content_Attachments, PermissionType.Edit);
        ...
        public JsonResult GetList(int pageIndex = 1, int pageSize = 15)
        {
            ...
            var where = LambdaHelper.True<Attachments>().And(p => !string.IsNullOrWhiteSpace(p.Url));
        ...
        [HttpPost]
        [PermissionFilter(MenuCode.Content_Attachments, PermissionType.Delete)]
        public IActionResult Delete(long id)
        {
            var attachment = _repository.GetOne(id);
            if (attachment == null)
            {
            ...
            {
                var filePath = Path.Combine(_environment.WebRootPath, attachment.Url.TrimStart('/'));
